...
&mdash;&nbsp;она принимает такие аргументы clamp([проверяемое значение], [минимальное значение], [максимальное значение]);
&mdash;&nbsp;возвращает [проверяемое значение], если оно в&nbsp;пределах второго и&nbsp;третьего аргумента. Если [проверяемое значение] выход за&nbsp;пределы минимума и&nbsp;максимума, то&nbsp;возвращает минимум или максимум соответственно;
...
Если ```fresnelTerm``` &lt; 0.95&nbsp;&mdash; значит что пора рисовать подсветку. При больших значениях рисуем как обычно.
...
Рис.&nbsp;3. Свечение атмосферы с&nbsp;обратной стороны
...
```ProceduralTexture```&mdash;такой вид текстуры позволяет нам генерировать ее&nbsp;визуализацию в&nbsp;GPU через фрагментный шейдер.
...
Рис.&nbsp;2. Эффект &laquo;god rays&raquo;.
...
Рис.&nbsp;3. Эффект &laquo;god rays&raquo; после настройки.
...
&mdash;&nbsp;```focus``` &mdash; это коэффициент растяжения круга по&nbsp;одной из&nbsp;осей, если оно равно&nbsp;1, то&nbsp;траектория будет не&nbsp;эллиптическая, а&nbsp;круговая

&mdash;&nbsp;```r``` &mdash; базовый радиус вращения Луны. Рассчитывается, как дистанция между точками статическим методом BABYLON.Vector2.Distance(vec2, vec2).
...
Рис.&nbsp;1. Наш результат&nbsp;&mdash; готовая сцена космоса.
